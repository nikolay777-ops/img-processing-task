...
{
private:
    std::vector<int> ComputeHistogram(const std::vector<std::vector<float>>& image);
    std::vector<int> ComputeCumulativeSum(const std::vector<int>& input);
    float ComputeMeanIntensity(const std::vector<int>& histogram);
    float ComputeOtsuThreshold(const std::vector<std::vector<float>>& image);
    void BinaryThreshold(const std::vector<std::vector<float>>& inputImage, std::vector<std::vector<int>>& outputImage, float threshold);

    Binarization() {};

public:
    static void OtsuThreshold(const std::vector<std::vector<float>>& inputImage, std::vector<std::vector<int>>& outputImage);
};
...
private:
    ...
    void BFS(int x, int y, int label, const std::vector<std::vector<int>>& binaryImg, std::vector<std::vector<int>>& labels);    
    
    Borders() {};

public:
    static void GetBoundingBox(const std::vector<std::pair<int, int>>& contour, int& minX, int& minY, int& maxX, int& maxY);
    static void CCA(const std::vector<std::vector<int>>& binaryImg, std::vector<std::vector<int>>& labels);
};
